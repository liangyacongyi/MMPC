...
def get_data_set(name="train", cifar=10, aug=False):
    ...
    if cifar == 10:
        ...
        f = open('/root/data/data_set/' + folder_name + '/batches.meta', 'rb')
        label_str = "label_names"
    else:
        folder_name = "cifar_100"
        f = open('/root/data/data_set/' + folder_name + '/meta', 'rb')
    ...
    if name is "train":
        if cifar == 10:
            for i in range(5):
                f = open('/root/data/data_set/' + folder_name + '/data_batch_' + str(i + 1), 'rb')
                datadict = pickle.load(f, encoding='latin1')
                f.close()
                _X = datadict["data"]
                _Y = datadict['labels']
            ...
            f = open('/root/data/data_set/' + folder_name + '/train', 'rb')
            datadict = pickle.load(f, encoding='latin1')
            f.close()
            _X = datadict["data"]
        ...
        if cifar == 10:
            f = open('/root/data/data_set/' + folder_name + '/test_batch', 'rb')
            label_str = "labels"
        else:
            f = open('/root/data/data_set/' + folder_name + '/test', 'rb')
...
def maybe_download_and_extract(cifar):
    ...
    :return: none
    """
    main_directory = "/root/data/data_set/"
